...
bad <- c('some_college',
         'pop',
         'white_not_hispanic',
         'households',  # not 0 but small enough to basically not count
...
this time the lasso got rid of a lot more variables than ebfore, that's kinda good i guess
```{r}
...
damn, it seems `some_college` didn't make the cut this time as well, i wonder why that is
```{r}
fit_ue1 <- lm(
        rate ~ hs_grad + bachelors + black + native + white_not_hispanic + age_under_5 + women_16_to_50_birth_rate + veterans + computer + uninsured_age_under_19,
        unemp_data
)
summary(fit_ue1)
```

and here's the final fit with all the definitely significant variables
```{r}
fit_ue2 <- lm(
        rate ~ hs_grad + bachelors + native + white_not_hispanic + age_under_5 + women_16_to_50_birth_rate + veterans + computer + uninsured_age_under_19,
        unemp_data
)
summary(fit_ue2)
```
